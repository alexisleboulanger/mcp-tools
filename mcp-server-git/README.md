...
#### Update repository path and start from mcp.json

1. Open your MCP configuration file:
   - User config: run `MCP: Open User Configuration` from the Command Palette.
   - Workspace config: edit `.vscode/mcp.json`.

2. Set the Git server entry and replace the repository path in `--repository`.

Using a local virtual environment (recommended for workspace isolation):
...
```json
{
  "servers": {
    "git": {
      "type": "stdio",
      "command": "C:\\path\\to\\mcp-server-git\\.venv\\Scripts\\python.exe",
      "args": [
        "-m",
        "mcp_server_git",
        "--repository",
...
3. Save the file. VS Code will start the server from this configuration when MCP servers are started or when a tool call needs the server.

4. If needed, restart MCP servers from the Command Palette (`MCP: List Servers` and restart `git`).
