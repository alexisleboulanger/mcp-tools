...
const toolDefinitions = [
  {
    name: 'list_agents',
    description: 'List all registered agents with their capabilities, delegation targets, and MCP server dependencies. Returns a summary of all Agent entities from the knowledge graph, enriched with agent file metadata.',
    inputSchema: {
      type: 'object',
      properties: {
        includeServers: {
...
function handleListAgents(args) {
  ...
  const response = { agents: result, totalAgents: result.length };
...
function handleAgentHealth(args) {
  ...
  const checkAgent = (agent) => {
    ...
    if (servedBy.length === 0) issues.push('No served_by relations (no MCP server connections)');
